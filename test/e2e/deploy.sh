...
vagrant ssh server -c "${BUILD_CMD} && sudo systemctl restart quiccochet-server && sleep 1 && echo \"quiccochet-server: \$(sudo systemctl is-active quiccochet-server)\" && quiccochet --version 2>&1 | head -1"

echo ""
echo "=== rebuild + restart on client ==="
vagrant ssh client -c "${BUILD_CMD} && sudo systemctl restart quiccochet-client && sleep 2 && echo \"quiccochet-client: \$(sudo systemctl is-active quiccochet-client)\" && quiccochet --version 2>&1 | head -1 && ss -tlnp | grep -q ':1080' && echo 'SOCKS5: up' || echo 'SOCKS5: DOWN'"
